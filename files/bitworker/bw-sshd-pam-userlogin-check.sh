#!/bin/bash
#
# bw-sshd-pam-userlogin-check.sh
# PAM exec authentication script for sshd (SFTP) against virtualx MySQL db.
#
# Called by pam_exec with expose_authtok; password arrives on stdin.
# PAM_USER is set as environment variable by PAM.
#
# Deployed to: /etc/bitworker/bw-sshd-pam-userlogin-check.sh (chmod 700, root:root)
...
#
# Test on terminal:
# echo 'PASSWORD' | PAM_USER=srv016.host-x.de /etc/bitworker/bw-sshd-pam-userlogin-check.sh --debug; echo "Exit code: $?"
...
if [[ "${BW_SSHD_DEBUG:-0}" == "1" || "${1:-}" == "--debug" ]]; then
    debug_enabled=1
fi

debug_log() {
    if [[ "$debug_enabled" -eq 1 ]]; then
        printf 'bw-sshd-pam-userlogin-check: %s\n' "$1" >&2
    fi
}
