...
if [ "$u_dnf" = "y" ]; then
    ...
    if [ -f "$file001" ]; then
        ...
        printf "[\e[32mOK\e[0m] dnf-automatic.conf updated\n"

        if systemctl enable --now dnf-automatic.timer; then
            printf "[\e[32mOK\e[0m] dnf-automatic.timer enabled\n"
        else
