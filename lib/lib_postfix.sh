...
printf "\n********************************************************************\n\nConfigure /etc/postfix/main.cf %s: " "$_postfix_prompt"
if [ "$u_postfix" = "" ]; then
    read u_postfix
...
if [ "$u_postfix" = "y" ]; then
    ...
    cp $u_path/files/postfix/mysql-domains.cf /etc/postfix/ && printf "[\e[32mOK\e[0m] mysql-domains.cf copied\n"
    cp $u_path/files/postfix/mysql-virtual.cf /etc/postfix/ && printf "[\e[32mOK\e[0m] mysql-virtual.cf copied\n"
...
# inet_protocols = ipv4 combined with inet_interfaces = all requires a full
      ...
      if systemctl stop postfix && systemctl start postfix; then
        printf "[\e[32mOK\e[0m] Postfix restarted\n"
    else
        printf "[\e[31mERROR\e[0m] Postfix restart failed\n"
    fi
