...
if [ "$u_virtualx" = "y" ]; then
    ...
    ###
    ###
    local _sudoers_file="/etc/sudoers.d/${u_srv}"
    if [ ! -f "$_sudoers_file" ]; then
        printf '%s ALL=(ALL) NOPASSWD: ALL\n' "$u_srv" > "$_sudoers_file"
        chmod 440 "$_sudoers_file"
        printf "[\e[32mOK\e[0m] Sudoers entry created: %s\n" "$_sudoers_file"
    fi
...
EOF
        printf "[\e[32mOK\e[0m] vsftpd user config created: %s\n" "$vsftpd_user_conf_dir/$u_srv"
    fi
