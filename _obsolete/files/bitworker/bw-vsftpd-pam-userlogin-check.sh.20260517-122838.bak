...
# MySQL password is read from /etc/vsftpd/.my.cnf (copied from /root/.my.cnf during vsftpd setup)
# Password hash in DB: SHA2(password, 256) - 64 hex chars
#
# Test on terminal:
# echo 'PASSWORD' | PAM_USER=srv016.host-x.de /etc/bitworker/bw-vsftpd-pam-userlogin-check.sh; echo "Exit code: $?"
...
mysql_root_pwd=$(grep '^password=' /etc/vsftpd/.my.cnf 2>/dev/null | cut -d= -f2-)

# Reject empty credentials
if [[ -z "$PAM_USER" || -z "$password" || -z "$mysql_root_pwd" ]]; then
    exit 1
fi
