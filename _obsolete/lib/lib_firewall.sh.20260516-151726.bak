...
if [ "$u_firewall" = "y" ]; then
    ...
    printf "[\e[32mOK\e[0m] /etc/firewall configured\n"
...
if [ "$u_activate_firewall" = "y" ]; then
    ...
    printf "[\e[32mOK\e[0m] Firewall activated\n"
...
if confirm "Kernel-Hardening (ASLR, SYN-Flood, Redirect-/Spoofing-Schutz, Kernel-Pointer, Symlink-Schutz)" "$u_hardening"; then
    ...
    if ! cp $u_path/files/firewall/99-bw-kernel-hardening.conf $folder001; then
        printf "[\e[31mERROR\e[0m] cp 99-bw-kernel-hardening.conf failed\n"
    else
        ### activate kernel rules
        ###
        ###
        sysctl -p "$folder001/99-bw-kernel-hardening.conf"
        printf "[\e[32mOK\e[0m] Kernel hardening applied\n"
    fi
