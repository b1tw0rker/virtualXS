...
letsencrypt_show_cert_status() {
    local domain="$1"
    local cert_file="/etc/letsencrypt/live/${domain}/cert.pem"

    if [ ! -f "$cert_file" ]; then
        _log warn "No certificate found for: ${domain}"
        return
    fi
    ...
    if [ -z "$end_date" ]; then
        _log warn "Certificate found but could not read expiry for: ${domain}"
        return
    fi
    ...
    if [ "$days_left" -lt 0 ]; then
        _log error "Certificate for ${domain} EXPIRED ${days_left#-} days ago (${end_date})"
    elif [ "$days_left" -lt 30 ]; then
        _log warn "Certificate for ${domain} expires in ${days_left} days (${end_date})"
    else
        _log ok "Certificate for ${domain} valid — expires in ${days_left} days (${end_date})"
    fi
}
...
if [ "$u_letsencrypt" = "y" ] || [ "$u_letsencrypt" = "d" ]; then
    printf "\n"
    letsencrypt_dns_prepare
...
if [ "$u_letsencrypt_dovecot" = "y" ] || [ "$u_letsencrypt_dovecot" = "d" ]; then
    printf "\n"
    ### ask for servername
    ...
    _log ok "Dovecot SSL cert configured"
...
if [ "$u_letsencrypt_dovecot_smtp" = "y" ] || [ "$u_letsencrypt_dovecot_smtp" = "d" ]; then
    printf "\n"
    ### ask for servername
    ...
    ###
    ###
    if [ "$u_letsencrypt_dovecot_smtp" = "d" ]; then
        letsencrypt_dns_issue "$u_smtp_dovecot" "true"
    else
        letsencrypt_dns_issue "$u_smtp_dovecot"
    fi
